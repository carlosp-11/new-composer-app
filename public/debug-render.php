...
echo "Laravel Root: $laravel_root<br>";
echo "Artisan: " . (file_exists($laravel_root . '/artisan') ? '✅ Encontrado' : '❌ No encontrado') . "<br>";
echo "Bootstrap: " . (is_dir($laravel_root . '/bootstrap') ? '✅ Existe' : '❌ No existe') . "<br>";
echo "Storage: " . (is_dir($laravel_root . '/storage') ? '✅ Existe' : '❌ No existe') . "<br>";
echo "Vendor: " . (is_dir($laravel_root . '/vendor') ? '✅ Existe' : '❌ No existe') . "<br>";

echo "<h2>4. Permisos</h2>";
if (is_dir($laravel_root . '/storage')) {
    echo "Storage writable: " . (is_writable($laravel_root . '/storage') ? '✅ SI' : '❌ NO') . "<br>";
    if (is_dir($laravel_root . '/storage/logs')) {
        echo "Storage/logs writable: " . (is_writable($laravel_root . '/storage/logs') ? '✅ SI' : '❌ NO') . "<br>";
    }
}
if (is_dir($laravel_root . '/bootstrap/cache')) {
    echo "Bootstrap/cache writable: " . (is_writable($laravel_root . '/bootstrap/cache') ? '✅ SI' : '❌ NO') . "<br>";
}
...
$log_path = $laravel_root . '/storage/logs/laravel.log';
